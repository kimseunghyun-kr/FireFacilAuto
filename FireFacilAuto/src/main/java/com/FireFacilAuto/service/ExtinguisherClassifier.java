...
public class ExtinguisherClassifier {
    ...
    public ExtinguisherInstallation classify(BuildTarget buildTarget) {
        ...
        extinguisherInstallation.waterSprayerEtCeteraApparatus = waterSprayerEtCeteraApparatusCheck();
        extinguisherInstallation.OutdoorFireHydrantApparatus = OutdoorFireHydrantApparatus();
     ...
     * 1 - 사
     */
    private Boolean OutdoorFireHydrantApparatus() {
//        옥외소화전설비를 설치해야 하는 특정소방대상물(아파트등, 위험물 저장 및
//        처리 시설 중 가스시설, 지하구 및 지하가 중 터널은 제외한다)은 다음의 어느
//        하나에 해당하는 것으로 한다.
        List<CO> exclusionList = Arrays.asList(make(1,1), make(17,2), make(17,3), make(17,4), make(27,2), make(28, -1));
        if(exclusionList.contains(btClassifier)){
            return false;
        }

//        1) 지상 1층 및 2층의 바닥면적의 합계가 9천㎡ 이상인 것. 이 경우 같은 구
//        (區) 내의 둘 이상의 특정소방대상물이 행정안전부령으로 정하는 연소(延燒)
//                우려가 있는 구조인 경우에는 이를 하나의 특정소방대상물로 본다.
        if(GFA >= 9000) {
            return true;
        }

//        2) 문화재 중 「문화재보호법」 제23조에 따라 보물 또는 국보로 지정된 목조
//                건축물

//        3) 1)에 해당하지 않는 공장 또는 창고시설로서 「화재의 예방 및 안전관리에
//        관한 법률 시행령」 별표 2에서 정하는 수량의 750배 이상의 특수가연물을 저
//        장ㆍ취급하는 것
        return false;
    }
     ...
     * 1 - 바
     */
    private Boolean waterSprayerEtCeteraApparatusCheck() {
//        물분무등소화설비를 설치해야 하는 특정소방대상물(위험물 저장 및 처리 시설
//        중 가스시설 및 지하구는 제외한다)은 다음의 어느 하나에 해당하는 것으로 한
//        다.
        List<CO> exclusionList = Arrays.asList(make(17,2), make(17,3), make(17,4), make(28, -1));
        if(exclusionList.contains(btClassifier)){
            return false;
        }
//        1) 항공기 및 자동차 관련 시설 중 항공기 격남고
        if (c == 18) {
            return true;
        }

//        2) 차고, 주차용 건축물 또는 철골 조립식 주차시설. 이 경우 연면적 800㎡ 이상
//        인 것만 해당한다.

//        3) 건축물의 내부에 설치된 차고ㆍ주차장으로서 차고 또는 주차의 용도로 사용
//        되는 면적이 200㎡ 이상인 경우 해당 부분(50세대 미만 연립주택 및 다세대주
//        택은 제외한다)

//        4) 기계장치에 의한 주차시설을 이용하여 20대 이상의 차량을 주차할 수 있는
//        시설

//        5) 특정소방대상물에 설치된 전기실ㆍ발전실ㆍ변전실(가연성 절연유를 사용하지
//                않는 변압기ㆍ전류차단기 등의 전기기기와 가연성 피복을 사용하지 않은 전
//                선 및 케이블만을 설치한 전기실ㆍ발전실 및 변전실은 제외한다)ㆍ축전지실ㆍ
//        통신기기실 또는 전산실, 그 밖에 이와 비슷한 것으로서 바닥면적이 300㎡ 이
//        상인 것[하나의 방화구획 내에 둘 이상의 실(室)이 설치되어 있는 경우에는
//        이를 하나의 실로 보아 바닥면적을 산정한다]. 다만, 내화구조로 된 공정제어
//        실 내에 설치된 주조정실로서 양압시설(외부 오염 공기 침투를 차단하고 내부
//                의 나쁜 공기가 자연스럽게 외부로 흐를 수 있도록 한 시설을 말한다)이 설치
//        되고 전기기기에 220볼트 이하인 저전압이 사용되며 종업원이 24시간 상주하
//        는 곳은 제외한다.

//        6) 소화수를 수집ㆍ처리하는 설비가 설치되어 있지 않은 중ㆍ저준위방사성폐기
//        물의 저장시설. 이 시설에는 이산화탄소소화설비, 할론소화설비 또는 할로겐화
//        합물 및 불활성기체 소화설비를 설치해야 한다.

//        7) 지하가 중 예상 교통량, 경사도 등 터널의 특성을 고려하여 행정안전부령으
//        로 정하는 터널. 이 시설에는 물분무소화설비를 설치해야 한다.

//        8) 문화재 중 「문화재보호법」 제2조제3항제1호 또는 제2호에 따른 지정문화
//        재로서 소방청장이 문화재청장과 협의하여 정하는 것
    ...
    private Boolean SimplifiedSprinklerApparatusCheck() {
        // 1 - 마 - 1
        // 공동주택 중 연립주택 및 다세대주택(연립주택 및 다세대주택에 설치하는 간
        //이스프링클러설비는 화재안전기준에 따른 주택전용 간이스프링클러설비를 설
        //치한다)
        if( c == 1 && (s == 2 || s == 3) ) {
            return true;
        }
...
//            나) 정신의료기관 또는 의료재활시설로 사용되는 바닥면적의 합계가 300㎡
//            이상 600㎡ 미만인 시설

//            다) 정신의료기관 또는 의료재활시설로 사용되는 바닥면적의 합계가 300㎡
//            미만이고, 창살(철재ㆍ플라스틱 또는 목재 등으로 사람의 탈출 등을 막기
//                    위하여 설치한 것을 말하며, 화재 시 자동으로 열리는 구조로 되어 있는
//                    창살은 제외한다)이 설치된 시설

//            4) 교육연구시설 내에 합숙소로서 연면적 100㎡ 이상인 경우에는 모든 층

//            5) 노유자 시설로서 다음의 어느 하나에 해당하는 시설
//            가) 제7조제1항제7호 각 목에 따른 시설[같은 호 가목2) 및 같은 호 나목부터
//            바목까지의 시설 중 단독주택 또는 공동주택에 설치되는 시설은 제외하며,
//                    이하 “노유자 생활시설”이라 한다]
//            나) 가)에 해당하지 않는 노유자 시설로 해당 시설로 사용하는 바닥면적의 합
//            계가 300㎡ 이상 600㎡ 미만인 시설
//            다) 가)에 해당하지 않는 노유자 시설로 해당 시설로 사용하는 바닥면적의 합
//            계가 300㎡ 미만이고, 창살(철재ㆍ플라스틱 또는 목재 등으로 사람의 탈출
//                    등을 막기 위하여 설치한 것을 말하며, 화재 시 자동으로 열리는 구조로
//                    되어 있는 창살은 제외한다)이 설치된 시설

//            6) 숙박시설로 사용되는 바닥면적의 합계가 300㎡ 이상 600㎡ 미만인 시설
//            7) 건물을 임차하여 「출입국관리법」 제52조제2항에 따른 보호시설로 사용하
//            는 부분
//            8) 복합건축물(별표 2 제30호나목의 복합건축물만 해당한다)로서 연면적 1천㎡
//            이상인 것은 모든 층
        }
        return false;
    }
